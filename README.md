...
**Name:** William Wert  
**NetID:** dxg9tt  
**DOI:** [https://doi.org/10.5281/zenodo.XXXXXXX](https://doi.org/10.5281/zenodo.XXXXXXX) ← *Replace with your Zenodo DOI after upload*  
...
The primary data source for this project is **PJM's Data Miner 2 portal** (dataminer2.pjm.com), the official open-access REST API maintained by PJM Interconnection LLC. PJM publishes verified, metered hourly load data for each of its transmission zones going back to the early 2000s. For this project, data is pulled from the **PJM East (PJM_E)** subzone via the `/hrl_load_metered` endpoint, selecting the `datetime_beginning_utc` and `area_load_mw` fields for a ten-year window (2014–2024). This results in approximately 87,600 hourly records. Each record is returned as a JSON object with a UTC timestamp and a megawatt value, mapping directly to a MongoDB document. The API is public, requires no authentication, and is rate-limited to approximately 1 request per second — which `ingest_pjm.py` respects with a configurable delay between requests.
